...
serve(async (req) => {
  ...
  try {
    ...
    if (project?.whitepaper_content && project?.whitepaper_extraction_status === 'extracted') {
      ...
      whitepaperText = project.whitepaper_content;
    }
    ...
    if (!content && whitepaperUrl && currentProjectId) {
      ...
      try {
        ...
        console.log(`✅ Fetched whitepaper: ${content.length} characters`);
...
2. Evidence evaluation - For each major piece of evidence or technical detail they provide to support their main claim, write a comprehensive evaluation that:
   - Describes what evidence they actually provide
   - Critically assesses whether this evidence supports their main claim
   - Compares their approach to similar existing systems (with specific examples and numbers)
   - Identifies what crucial information is missing
   - Explains why this evidence is strong, weak, or irrelevant to their main claim
