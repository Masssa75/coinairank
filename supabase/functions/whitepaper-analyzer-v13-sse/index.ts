...
async function processWithSSE(
  ...
  await sendEvent('starting', {
    message: `Starting V13 clear verdict analysis for ${symbol}...`
  });
  ...
  await sendEvent('ai_analyzing', {
    message: 'V13: Adding clear verdict to analysis...'
  });
...
CRITICAL ADDITION - CLEAR VERDICT:
End with a definitive conclusion that helps readers decide.

Structure:
- Start with "THE VERDICT:" followed by your assessment
- One clear sentence stating your position
- 2-3 sentences explaining why you reached this conclusion
- Be decisive - avoid hedging or "on one hand/other hand"
- Include a specific action or timeline if relevant

Extract:
1. The main claim as a clear news headline (one sentence)
2. Balanced analysis in 2-3 paragraphs
3. END WITH bold verdict that helps readers decide what to do
...
Output JSON:
{
  ...
  await sendEvent('saving', {
    message: 'Saving V13 clear verdict analysis...'
  });
  ...
  if (saveError) {
    console.error(`Failed to save V13 results: ${saveError.message}`);
      ...
      (async () => {
        try {
          ...
          await sendEvent('complete', {
            message: 'V13 clear verdict analysis complete',
            result
          });
        } catch (error) {
