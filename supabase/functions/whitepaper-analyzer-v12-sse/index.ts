...
async function processWithSSE(
  ...
  await sendEvent('starting', {
    message: `Starting V12 concrete metrics analysis for ${symbol}...`
  });
  ...
  await sendEvent('ai_analyzing', {
    message: 'V12: Adding concrete metrics to analysis...'
  });
...
CRITICAL ADDITION - CONCRETE METRICS:
Support all claims with specific data wherever possible.

Requirements:
- Include actual numbers, not just qualitative descriptions
- Provide context for numbers through comparisons
- Use percentages, ratios, or growth rates where relevant
- Cite timeframes for any predictions or milestones
- Quantify both opportunities and risks

Extract:
1. The main claim as a clear news headline (one sentence)
2. Balanced analysis in 2-3 paragraphs with SPECIFIC NUMBERS throughout
3. Every claim supported by concrete metrics where possible
...
Output JSON:
{
  ...
  await sendEvent('saving', {
    message: 'Saving V12 concrete metrics analysis...'
  });
  ...
  if (saveError) {
    console.error(`Failed to save V12 results: ${saveError.message}`);
      ...
      (async () => {
        try {
          ...
          await sendEvent('complete', {
            message: 'V12 concrete metrics analysis complete',
            result
          });
        } catch (error) {
