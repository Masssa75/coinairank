...
async function processWithSSE(
  ...
  const systemPrompt = 'You are an expert crypto whitepaper analyst focused on evaluating transformative potential. Consider what this project could mean for the entire crypto ecosystem if successfully implemented.';

  const userPrompt = `Analyze this whitepaper with a focus on TRANSFORMATIVE POTENTIAL.

Consider:
- What would this mean for the crypto ecosystem if successfully implemented?
- How fundamentally could this change existing paradigms?
- What cascading effects could this innovation trigger?
- How does this compare to other ecosystem-changing projects like Ethereum, Bitcoin, Chainlink, etc.?

Extract:
1. The main claim in one clear sentence
2. Evaluate the transformative potential in 2-3 paragraphs focusing on:
   - Revolutionary scope and ecosystem impact
   - Paradigm-shifting implications
   - Network effects and adoption potential
   - Comparison to other transformative crypto innovations
...
Output JSON:
{
  ...
  "claim_evaluation": "2-3 paragraph evaluation focused on transformative potential and ecosystem impact"
}`;
